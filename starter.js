...
let returnGreeting = formalNames.map((name) => {
    return "Hello, " + name;
});
...
const placesThatStartWithA = places.filter((name) => {
...
let identifier = (jobList) => {
    let filteredArr = jobList.filter((job) => {
        return job.programmer !== undefined
    });
...
// CODE HERE
const productOfArray = numsToReduce.reduce((acc, curr) => acc * curr);
